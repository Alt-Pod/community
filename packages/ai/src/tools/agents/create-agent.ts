...
import type { CommunityToolDefinition, ToolContext } from "../types";
    ...
    tool({
      ...
      execute: async ({ name, description, system_prompt, tool_ids }) => {
        const [agent] = await sql<Agent[]>`
          INSERT INTO agents (name, description, system_prompt, user_id)
          VALUES (${name}, ${description}, ${system_prompt}, ${ctx.userId})
